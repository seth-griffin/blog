...
def test_blog_route_test_rss_output_structure(client):
    ...
    for entry_node in entry_nodes:
        assert entry_node.find("atom:title", namespaces).text == "Introduction to the Arc Programming Language"
        
        assert entry_node.find("atom:link", namespaces).get("href") == "/programming/functional-programming/lisp/scheme/arc/12/11/2025/introduction-to-the-arc-programming-language.html"
        ...
        assert entry_node.find("atom:link", namespaces).get("title") == "Introduction to the Arc Programming Language"
        
        assert entry_node.find("atom:published", namespaces).text == "20251211T000000.000000Z"
        
        assert entry_node.find("atom:updated", namespaces).text == "20251211T000000.000000Z"
      
        assert entry_node.find("atom:id", namespaces).text.strip() == "/programming/functional-programming/lisp/scheme/arc/12/11/2025/introduction-to-the-arc-programming-language"
        
        assert entry_node.find("atom:content", namespaces).get("type") == "html"

        xmlns = "{http://www.w3.org/XML/1998/namespace}"
        assert entry_node.find("atom:content", namespaces).get(f"{xmlns}base") == "/programming/functional-programming/lisp/scheme/arc/12/11/2025/introduction-to-the-arc-programming-language.html"
